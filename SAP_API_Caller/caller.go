...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-process-order-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) General(manufacturingOrder string) {
	generalData, err := c.callProcessOrderSrvAPIRequirementGeneral("A_ProcessOrder_2", manufacturingOrder)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callProcessOrderSrvAPIRequirementGeneral(api, manufacturingOrder string) ([]sap_api_output_formatter.General, error) {
	url := strings.Join([]string{c.baseURL, "API_PROCESS_ORDER_2_SRV", api}, "/")
